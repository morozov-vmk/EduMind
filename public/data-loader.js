const DataLoader = {
    ...
    async loadTask(taskNumber) {
        ...
        try {
            console.log(`${this.basePath}/tasks/task${taskNumber}`);
            const response = await fetch(`${this.basePath}/tasks/task${taskNumber}`);
    ...
    async loadExamTask(taskNumber) {
        ...
        try {
            const response = await fetch(`${this.basePath}/exam/task${taskNumber}`);
            ...
            } else if (examData && Array.isArray(examData.tasks)) {
                normalized = examData;
                normalized.number = normalized.number || taskNumber;
            } else {
                normalized = examData;
                normalized.number = normalized.number || taskNumber;
            }

            this.cache.examTasks[taskNumber] = normalized;
            return normalized;
        } catch (error) {
            ...
            this.cache.examTasks[taskNumber] = fallback;
            return fallback;
        }
    },
    ...
    getCurrentTag() {
        return localStorage.getItem('current_tag') || 'loop_for';
    },
